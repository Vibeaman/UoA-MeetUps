...
export const ProfileDetailModal: React.FC<ProfileDetailModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-0 sm:p-4 bg-black/85 backdrop-blur-md overflow-y-auto">
      <div className="relative w-full max-w-lg bg-[#0c0517] border border-purple-900/40 rounded-none sm:rounded-3xl shadow-2xl overflow-hidden min-h-screen sm:min-h-0 sm:max-h-[92vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar pb-24">
          ...
          <div className="relative w-full h-[400px] sm:h-[440px] bg-black">
                ...
                {profile.photos.map((_, i) => (
                  <button
                    ...
                    className={`h-1.5 flex-1 rounded-full transition-all ${
                      i === photoIndex ? 'bg-purple-300 shadow-[0_0_8px_#c084fc]' : 'bg-black/60'
                    }`}
                  />
                ))}
              </div>
            ...
            <div className="absolute bottom-4 left-5 right-5 flex flex-wrap items-center gap-2 z-10">
              {profile.isVerified && (
                <span className="flex items-center space-x-1 px-3 py-1 rounded-full bg-purple-900/90 border border-purple-400 text-purple-100 text-xs font-bold shadow-md backdrop-blur-md">
                  <ShieldCheck className="w-4 h-4 text-purple-300" />
                  <span>UniAbuja Verified Student</span>
                </span>
              )}
              {profile.mode === 'lowkey' && (
                <span className="flex items-center space-x-1 px-3 py-1 rounded-full bg-fuchsia-950/90 border border-fuchsia-400 text-fuchsia-200 text-xs font-bold backdrop-blur-md">
          ...
          <div className="p-5 sm:p-6 space-y-6">
            ...
            <div>
              <div className="flex items-baseline justify-between">
                <h1 className="text-3xl font-black font-display text-white">
                  {profile.name} <span className="text-2xl font-light text-purple-300">{profile.age}</span>
                </h1>
              ...
              <div className="mt-2 flex flex-col space-y-1">
                <div className="flex items-center space-x-2 text-sm text-purple-200 font-semibold">
                  <GraduationCap className="w-4 h-4 text-purple-400" />
                  <span className="bg-purple-950 px-2 py-0.5 rounded text-xs text-purple-300 border border-purple-800/40">
                    {profile.level}
                  </span>
                ...
                <div className="flex items-center space-x-2 text-xs text-neutral-400 pl-6 mt-0.5">
                  <MapPin className="w-3.5 h-3.5 text-purple-400 shrink-0 -ml-5" />
            ...
            {/* Looking For Card */}
            <div className="p-4 rounded-2xl bg-[#140a24] border border-purple-900/40 flex items-center justify-between">
              <div>
                <span className="text-xs text-neutral-400 uppercase tracking-wider font-semibold">Looking For</span>
                <p className="text-sm font-bold text-purple-200 capitalize mt-0.5">
            ...
            {/* Bio Section */}
            <div>
              <h3 className="text-xs uppercase font-bold tracking-wider text-purple-400 mb-2">About Me</h3>
              <p className="text-neutral-200 text-sm leading-relaxed whitespace-pre-line bg-[#10061d] p-4 rounded-2xl border border-purple-950">
                {profile.bio}
              </p>
            </div>
            ...
            {profile.icebreakerPrompts && profile.icebreakerPrompts.length > 0 && (
              <div className="space-y-3">
                <h3 className="text-xs uppercase font-bold tracking-wider text-purple-400 flex items-center space-x-1.5">
                ...
                {profile.icebreakerPrompts.map((prompt) => (
                  <div
                    key={prompt.id}
                    className="p-4 rounded-2xl bg-gradient-to-br from-[#180b2c] to-[#120722] border border-purple-800/40 shadow-sm"
                  >
            ...
            {profile.photos.length > 1 && (
              <div className="space-y-2">
                <h3 className="text-xs uppercase font-bold tracking-wider text-purple-400">More Photos</h3>
                <div className="grid grid-cols-2 gap-2">
                  {profile.photos.slice(1).map((photo, idx) => (
                    <div
                      key={idx}
                      onClick={() => setPhotoIndex(idx + 1)}
                      className="relative h-44 rounded-2xl overflow-hidden border border-purple-950 cursor-pointer group"
                    >
                      <img
                        src={photo}
                        alt=""
            ...
            {profile.interests && profile.interests.length > 0 && (
              <div>
                <h3 className="text-xs uppercase font-bold tracking-wider text-purple-400 mb-2">Campus Interests</h3>
                <div className="flex flex-wrap gap-2">
                  {profile.interests.map((tag, idx) => (
                    <span
                      key={idx}
                      className="px-3 py-1.5 rounded-full bg-purple-950/70 border border-purple-800/50 text-xs font-medium text-purple-200"
                    >
                      {tag}
                    </span>
                  ))}
            ...
            <div className="p-4 rounded-2xl bg-purple-950/40 border border-purple-800/30 flex items-start space-x-3">
        ...
        <div className="absolute bottom-0 inset-x-0 p-4 bg-gradient-to-t from-[#090410] via-[#090410]/95 to-transparent border-t border-purple-950/80 flex items-center justify-center space-x-6 backdrop-blur-xl z-40">
          <button
            onClick={handlePass}
          ...
          <button
            onClick={handleLike}
            className="flex-1 py-3 px-4 rounded-2xl bg-gradient-to-r from-purple-600 to-fuchsia-600 text-white font-bold text-sm flex items-center justify-center space-x-2 shadow-lg shadow-purple-900/50 hover:brightness-110 transition-all"
