...
export const AdminAccessGate: FC<AdminAccessGateProps> = ({ onBack }) => {
  ...
  return (
    <main className="w-full flex-1 flex items-center justify-center px-4 py-10 relative z-10">
      <div className="w-full max-w-sm rounded-[2rem] border border-purple-700/50 bg-[#120620]/95 p-6 sm:p-8 shadow-[0_20px_80px_rgba(88,28,135,0.28)]">
        <button
          type="button"
          onClick={onBack}
          className="mb-8 inline-flex items-center gap-2 rounded-xl border border-purple-900/70 bg-purple-950/40 px-3 py-2 text-xs font-bold text-purple-200 transition-colors hover:border-purple-600 hover:text-white"
        >
          <ArrowLeft className="h-4 w-4" />
          Return to app
        </button>

        <div className="mb-6 flex h-14 w-14 items-center justify-center rounded-2xl border border-purple-500/50 bg-gradient-to-br from-purple-600/30 to-fuchsia-600/20 text-purple-200 shadow-[0_0_24px_rgba(168,85,247,0.28)]">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm leading-6 text-purple-200/75">
            The moderation console is restricted to authorised UniAbuja staff.
          </p>
        </div>

        <form onSubmit={handleSubmit} className="mt-7 space-y-4">
          <label htmlFor="admin-password" className="block text-xs font-black uppercase tracking-[0.18em] text-purple-300">
            Staff password
          </label>
          <input
            id="admin-password"
            ...
            spellCheck={false}
            autoFocus
            className="w-full rounded-2xl border border-purple-800/70 bg-[#090312] px-4 py-3 text-center text-xl font-black uppercase tracking-[0.55em] text-white outline-none transition-colors placeholder:text-purple-300/30 focus:border-purple-400 focus:ring-2 focus:ring-purple-500/20"
